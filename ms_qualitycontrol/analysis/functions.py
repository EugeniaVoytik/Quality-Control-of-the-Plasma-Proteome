...
def get_list_of_col(df, searched_value):
    if len(searched_value) == 1:
        columns = [column for column in df.columns if searched_value[0] in column.lower()]
        return columns
    elif len(searched_value) > 1:
        columns = []
        for each in searched_value:
            columns.append([column for column in df.columns if each in column.lower()])
        return list(itertools.chain.from_iterable(columns))


def get_reversed_list_of_col(df, searched_value):
    if len(searched_value) == 1:
        columns = [column for column in df.columns if searched_value[0] not in column.lower()]
        return columns
    elif len(searched_value) > 1:
        columns = []
        for each in searched_value:
...
def get_list_of_indices(df, column, mask, separator):
    """
    A function that can be used for sorting of Data Frame based on a created list of values(mask).
    Args:
...
def extract_val_from_col(df, column, separator):
    ...
    """
    values_protIDs = [value.split(separator) for value in df[column].dropna(axis=0)]
...
def make_annotation_item(x, y, z, color):
    return {'xref': 'x', 'yref': 'y', 'x': x, 'y': y, 'text': '#%s' % z.replace('LFQ', '').replace(' intensity', ''),
            'showarrow': True, 'font': dict(family='Arial', size=12, color='white'),
            'align': 'left', 'arrowhead': 2, 'arrowsize': 1, 'arrowwidth': 2, 'arrowcolor': '#A9A9A9',
            'ax': 20, 'ay': -30, 'bordercolor': '#A9A9A9', 'borderwidth': 2, 'borderpad': 4, 'bgcolor': color,
            'opacity': 0.8}
...
def create_annotations(data, threshold, col_names):
    ...
    annotations = []
    for x, y, z in zip(data_above_threshold_indices, data_above_threshold, names):
